...
export const DEFAULT_CHAIN_DEADLINE_MS = 150000;
// Hoe lang een provider die in een eerdere aanroep volledig faalde wordt
// overgeslagen (per warm instance). Voorkomt dat batch 2 en 3 dezelfde
// dode provider opnieuw 20-40 seconden laten opeten.
export const PROVIDER_COOLDOWN_MS = Number(process.env.PROVIDER_COOLDOWN_MS) || 10 * 60 * 1000;

const TIMEOUTS = {
  mistral: 20000,
  geminiInteractions: 45000,
  geminiGenerateContent: 45000,
  openrouter: 30000
};
...
export function getMistralModels() {
  ...
  const fallback = process.env.AI_FALLBACK_MODEL || 'ministral-3b-latest';
...
export function getChainConfig() {
  return {
    mistralEnabled: process.env.MISTRAL_ENABLED !== 'false',
...
// ── Provider-cooldown ───────────────────────────────────────
const providerCooldowns = new Map(); // provider → tijdstip waarop de cooldown eindigt

function activeProviderCooldowns(now = Date.now()) {
  for (const [provider, expiresAt] of providerCooldowns) {
    if (expiresAt <= now) providerCooldowns.delete(provider);
  }
  return providerCooldowns;
}

// Zet providers die in deze run alleen maar gefaald hebben op cooldown;
// een succes in deze run wist de cooldown van die provider weer.
function applyProviderCooldowns(attempts) {
  const perProvider = new Map(); // provider → { failed, ok }
  for (const a of attempts) {
    const entry = perProvider.get(a.provider) || { failed: false, ok: false };
    if (a.ok) entry.ok = true; else entry.failed = true;
    perProvider.set(a.provider, entry);
  }
  for (const [provider, { failed, ok }] of perProvider) {
    if (ok) {
      if (providerCooldowns.delete(provider)) {
        console.warn(`Provider ${provider} reageert weer; cooldown opgeheven.`);
      }
      continue;
    }
    if (failed && !activeProviderCooldowns().has(provider)) {
      providerCooldowns.set(provider, Date.now() + PROVIDER_COOLDOWN_MS);
      console.warn(`Provider ${provider} gaat ${Math.round(PROVIDER_COOLDOWN_MS / 60000)} minuten op cooldown na volledig falen in deze aanroep.`);
    }
  }
}

// Alleen toepassen als er daadwerkelijk andere providers overblijven;
// een enkele provider mag nooit de hele keten leegmaken.
function skipCoolingDownProviders(steps) {
  const cooldowns = activeProviderCooldowns();
  if (!cooldowns.size || !steps.length) return steps;

  const remaining = steps.filter((step) => !cooldowns.has(step.provider));
  if (!remaining.length) return steps;

  const skipped = [...new Set(steps.filter((step) => cooldowns.has(step.provider)).map((step) => step.provider))];
  console.warn(`LLM-provider(s) overgeslagen door cooldown na eerdere fout: ${skipped.join(', ')}`);
  return remaining;
}
...
export async function buildChain() {
  ...
  // Mistral tijdelijk uit te schakelen met MISTRAL_ENABLED=false (standaard aan),
  // zodat Gemini de eerste provider is. Code blijft intact.
  const mistralEnabled = process.env.MISTRAL_ENABLED !== 'false';

  if (process.env.VIBE_CLI_KEY_BCG && mistralEnabled) {
    ...
    if (fallback && fallback !== primary) {
      // Fallback-model: bewust 1 poging van 20s (bespaart ketentijd; primaire
      // model houdt MAX_ATTEMPTS_PER_STEP).
      mistralSteps.push({ provider: 'mistral', model: fallback, transport: 'chat', timeoutMs: TIMEOUTS.mistral, maxAttempts: 1, call: (p, t) => callMistral(p, { model: fallback, timeoutMs: t }) });
    }
  ...
  return skipCoolingDownProviders(steps);
}
...
export async function runLlmChain(prompt, { deadlineMs = DEFAULT_CHAIN_DEADLINE_MS, onAttempt } = {}) {
  ...
  for (const step of steps) {
    ...
    for (let attempt = 1; attempt <= (step.maxAttempts ?? MAX_ATTEMPTS_PER_STEP); attempt++) {
      if (Date.now() >= deadlineAt) break;

      const maxAttempts = step.maxAttempts ?? MAX_ATTEMPTS_PER_STEP;
      ...
      try {
        ...
        onAttempt?.(record);
        applyProviderCooldowns(attempts);
        ...
        if (attempt < maxAttempts && Date.now() + backoffMs < deadlineAt) {
